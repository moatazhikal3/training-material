...
const slides = [
  {
    ...
    title: "The Trucking Industry & Its Players",
    layout: "title",
    icon: <Icon.Users className="w-12 h-12" style={{ color: brand.red }} />,
    trainerNotes: [
      "Welcome & context: In this section we'll map the major stakeholders in trucking: freight owners (shippers + receivers), freight brokers, and motor carriers. Dispatchers operate on the carrier side.",
      "Why it matters: Knowing who wants what helps us negotiate better, pick profitable loads, and set realistic expectations.",
      "Roles boundary: Remember: the broker is a counterpart, not our client; our client is the carrier.",
      "Navigation cue (first slide): You'll see a small navigation box here and on the final slide — use it to go to the next section or back to the previous one."
    ]
  },
  {
    title: "Key Concepts",
    ...
    table: {
      headers: ["Concept", "Summary / Implication"],
      rows: [
        ["Freight owners", "Shippers (send) + Receivers (receive); they own the freight."],
        ["Freight brokers", "Arrange transport; profit from spread (e.g., $1,100 vs $900 → $200)."],
        ["Asset-based brokers", "Own some trucks; cover part with fleet, broker the rest."],
        ["Brokers vs Dispatchers", "Brokers sell loads; dispatchers advocate carrier revenue — interests differ."],
        ["Motor carriers", "Own trucks/authority; dispatcher works on carrier side (employee/contractor)."],
        ["Carrier types", "Contract (specific customers) vs Common (general public); most dispatch work is common."]
      ]
    },
    trainerNotes: [
      "Freight owners: Define shippers (senders) and receivers (consignees). Confirm they collectively own the freight.",
      "Freight brokers: Licensed intermediaries who arrange transportation. Explain the profit 'spread' briefly (e.g., customer pays 1100, carrier paid 900 → broker keeps 200).",
      "Asset-based brokers: Brokers that also own trucks — they can cover some loads with their own fleet and broker out the rest.",
      "Brokers vs. Dispatchers: Brokers sell loads; dispatchers advocate for the carrier's revenue and schedule. Interests overlap but are not identical.",
      "Motor carriers (carriers): Trucking companies with authority to haul; dispatchers work for this side (employee or contractor).",
      "Carrier types: Contract vs. Common. Most dispatch work centers on common carriers (serving the general public).",
      "Checks for understanding: Who actually owns the freight? (Answer: the shipper/receiver, not the broker or carrier.) Where does broker profit come from? (Answer: the spread.) Who do dispatchers serve? (Answer: the carrier.)",
      "Transition: Now let's connect players to what they want and what that means for our day-to-day choices."
    ]
  },
  {
    title: "Players & What They Want",
    layout: "table",
    icon: <Icon.Users className="w-12 h-12" style={{ color: brand.red }} />,
    table: {
      headers: ["Player", "What they want / Implication for Dispatcher"],
      rows: [
        ["Freight Owners (Shipper/Receiver)", "Reliable capacity and fewer logistics headaches → Often prefer brokers for coverage; expect professional handling."],
        ["Brokers", "Earn the spread → They push to pay carriers less; dispatcher must negotiate assertively."],
        ["Asset-based Brokers", "Cover some freight with their own trucks → They still broker out remainder; know they have options."],
        ["Motor Carriers (Your Client)", "Profitable loads and consistent routing → Dispatcher optimizes revenue and timing."],
        ["Common vs Contract Carriers", "Common serve the public; Contract serve specific customers → Expect broader load types for common; specialized/niche work for some contract lanes."],
        ["Interstate vs Intrastate", "Interstate = between states; Intrastate = within a state → Most dispatchers work interstate; intrastate can build strong regional expertise."]
      ]
    },
    trainerNotes: [
      "Freight Owners (Shipper/Receiver): Want reliable capacity and fewer logistics headaches → Often prefer brokers for coverage. Dispatcher takeaway: brokers are frequent counterparties; be professional and clear.",
      "Brokers: Earn the spread → Expect them to push down carrier pay. Dispatcher takeaway: negotiate assertively, know your numbers.",
      "Asset-based Brokers: Can cover part of freight with their own trucks → They still broker overflow; they have options. Takeaway: justify your ask with market data and service quality.",
      "Motor Carriers (Your Client): Want profitable loads and predictable routing → You protect revenue/time and reduce empty miles.",
      "Common vs. Contract Carriers: Common = general public; Contract = specific customer(s) → Common brings variety; contract can be niche/specialized.",
      "Interstate vs. Intrastate: Interstate = between states; Intrastate = within one state → Most dispatchers handle interstate for broader opportunity; intrastate can build deep regional expertise.",
      "Prompt the room: Given a low initial broker offer, what data would you use to push for a higher rate? When would a carrier choose a contract lane over common freight?"
    ]
  },
  {
    title: "Pop Quiz",
    ...
    quiz: {
      questions: [
        "Who are freight owners?",
        "How do brokers earn profit?",
        "What's an asset-based broker?",
        "Who is the dispatcher's client, and why does it matter?",
        "Contract vs. Common carriers — what's the difference?",
        "Interstate vs Intrastate — which do most dispatchers work with, and why?"
      ],
      answers: [
        "Shipper + Receiver (they own the freight).",
        "From the spread between customer price and carrier pay (e.g., $1,100 vs $900 → $200).",
        "A broker that owns trucks and covers part of freight with its own assets.",
        "The carrier; the broker is a counterpart, not your client—your job is to protect carrier revenue and reliability.",
        "Contract serves specific customers; Common serves the general public (most dispatch work).",
        "Interstate mostly, because it demands market analysis across regions and provides more opportunity; intrastate builds deep regional expertise."
      ]
    },
    trainerNotes: [
      "Instructions: Ask each question. Get 1–2 volunteer answers. Only then toggle Trainer Mode to show model answers.",
      "If answers are thin, revisit Key Concepts or Players table briefly before revealing.",
      "Coaching: Reinforce 'carrier is the client.' Emphasize the spread and why negotiation is critical. Encourage concise definitions and real-world examples."
    ]
  },
  {
    ...
    bullets: [
      ...
      "Trainer script for materials will be pulled from the Section 2 notes when provided."
    ],
    trainerNotes: [
      "State of materials: There's no uploaded material for Section 2 yet. When we receive PDFs, images, or links, they'll appear here.",
      ...
      "Trainer script (when materials arrive): We'll follow the step-by-step instructions provided with the materials — for a website, we'll walk through where to click and what to verify; for a document, we'll point out the key sections to review.",
      "Time-box the walkthrough and summarize takeaways tied to negotiation/profitability or scheduling.",
...
// ===== Main App =====
export default function LoadisticsSection2({ onNavigateToSection, sectionDropdown }) {
  ...
  return (
    <div className="min-h-screen w-full bg-white text-gray-900 flex flex-col relative" style={{ fontFamily: "Inter, ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto" }}>
                ...
                {(slideIndex === 0 || slide.isMaterialsSlide) && onNavigateToSection && (
                  <div className="mt-6 p-4 bg-gray-50 rounded-xl border">
                    ...
                    <div className="flex gap-3">
                      <Button variant="outline" onClick={() => onNavigateToSection(1)} className="rounded-xl">
                        ← Previous Section (Section 1)
                      </Button>
                      <Button onClick={() => onNavigateToSection(3)} className="rounded-xl">
      ...
      <div className="p-4 text-center text-xs text-gray-400">© {new Date().getFullYear()} Loadistics LLC — Section 2</div>
    </div>
  );
}
